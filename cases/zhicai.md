...
**Gemini、ChatGPT、GitHub Copilot、Cursor**，由本人补充确认。它们用于辅助需求梳理、代码理解、实现与调试；具体功能是否符合采购流程、数据是否正确以及能否交付，由我检查和验证。不按工具品牌虚构各模块分工。

本案例确认的是采购业务应用；具体采用 Web（B/S）还是安装式客户端（C/S），现有实现资料仍不足，暂不指定架构。
## 业务问题与需求
...
可确认的本人贡献是主导工具开发、历史数据整理建库及上述业务模块实现。项目技术可确认到 Excel 数据处理、结构化数据库和业务应用层；前后端语言、框架、数据库产品与部署环境尚未核实，不从个人项目技术栈推断企业实现。
